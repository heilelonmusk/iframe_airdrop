...
def update_file(file_path, new_csv, sha):
    """
    Prova a utilizzare update_file sul repository.
    Se fallisce (ad esempio, perché il metodo non è disponibile), tenta un fallback: elimina e ricrea il file.
    """
    try:
        ...
        try:
            # Se il file esiste, cancellalo tramite il metodo delete() sull'oggetto file_content
...
def update_whitelist(wallet_address):
    """
    Controlla se il wallet è presente in data/whitelist.csv.
    Se presente, imposta 'Checked' a "true" e aggiorna il timestamp.
    ...
    if update_file(file_path, new_csv, file_content.sha):
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        sys.exit(1)
    wallet = sys.argv[1]
    # Prima, prova ad aggiornare la whitelist.
    if update_whitelist(wallet):
        # Se il wallet è presente nella whitelist, l'aggiornamento è completato.
        sys.exit(0)
    else:
        # Se il wallet non è presente nella whitelist, aggiungilo a non_eligible.csv.
